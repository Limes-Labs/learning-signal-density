#!/usr/bin/env python3
"""Audit scarce-label active acquisition on Twenty Newsgroups."""
...
def _teacher_prediction_margin(
    teacher: MulticlassPerceptronClassifier,
    labels: tuple[str, ...],
    text: str,
) -> tuple[str, float]:
...
def _seed_budget(train_budget: int, label_count: int) -> int:
    return min(train_budget, max(label_count, train_budget // 4))


def _select_acquisition_rows(
    ...
    if acquire_count <= 0:
        return []
    if mode == "margin_uncertainty":
        return sorted(scored_rows, key=lambda row: (row[2], row[0].token_count, row[0].record_id))[:acquire_count]
    if mode == "short_margin_uncertainty":
        return sorted(scored_rows, key=lambda row: (row[2] * max(1, row[0].token_count), row[0].token_count, row[0].record_id))[
            :acquire_count
        ]
    if mode == "confidence_curriculum":
        return sorted(scored_rows, key=lambda row: (row[2], -row[0].token_count, row[0].record_id), reverse=True)[
            :acquire_count
        ]
    if mode != "balanced_margin_uncertainty":
    ...
    target_counts = _balanced_counts(sorted(by_prediction), acquire_count)
...
def run_active_acquisition_condition(
    ...
    acquire_count = min(len(scored_rows), train_budget - len(seed_records))
    acquired_rows = _select_acquisition_rows(scored_rows, acquire_count, acquisition_mode)
    ...
    acquisition_scoring_cost_tokens = unlabeled_pool_tokens
    ...
    condition = f"class_balanced_seed_active_{acquisition_mode}"
...
def build_active_acquisition_audit(
    records: tuple[NewsRecord, ...],
    ...
    for train_budget in train_budgets:
        ...
        for seed in seeds:
            for mode in acquisition_modes:
                row = run_active_acquisition_condition(
                    records=records,
                    seed=seed,
                    train_budget=train_budget,
        ...
        reference_random = {"condition": "random_sample"} | source_artifact["budgets"][str(train_budget)]["conditions"][
            "random_sample"
        ]
        reference_class = {"condition": "class_balanced_sample"} | source_artifact["budgets"][str(train_budget)][
            "conditions"
    ...
    return {
        "title": "Twenty Newsgroups Active Label-Acquisition Audit",
        ...
        "condition_scope": {
            "class_balanced_seed_active_acquisition": {
                ...
                "acquisition_signal": "teacher margin over train-only unlabeled pool",
                "heldout_used_for_selection": False,
                "validation_used_for_selection": False,
                "oracle_train_labels_used_for_acquisition": False,
                "true_labels_acquired_after_selection": True,
                "cost_model": (
                    "class-balanced seed label-index cost plus teacher training, unlabeled-pool scoring, "
    ...
    for budget in artifact["train_budgets"]:
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    str(budget),
                    best_condition.replace("class_balanced_seed_active_", ""),
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("results/twenty_newsgroups_active_acquisition_audit.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("results/twenty_newsgroups_active_acquisition_audit.md"),
    )
    args = parser.parse_args()
    records = load_twenty_newsgroups_records(cache_path=args.cache_path)
    source_artifact = load_json(args.source_json)
    artifact = build_active_acquisition_audit(records=records, source_artifact=source_artifact)
