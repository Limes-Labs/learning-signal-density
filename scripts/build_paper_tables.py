...
TWENTY_NEWSGROUPS_LENGTH_WINDOW_CONFIRMATION_AUDIT_ARTIFACT = Path(
    "results/twenty_newsgroups_length_window_confirmation_audit.json"
)
TWENTY_NEWSGROUPS_FRONTIER_ROBUSTNESS_AUDIT_ARTIFACT = Path(
    "results/twenty_newsgroups_frontier_robustness_audit.json"
)
TWENTY_NEWSGROUPS_FRONTIER_FRESH_SEED_AUDIT_ARTIFACT = Path(
    "results/twenty_newsgroups_frontier_fresh_seed_audit.json"
)
TWENTY_NEWSGROUPS_CLASS_BALANCED_CONFIRMATION_AUDIT_ARTIFACT = Path(
    "results/twenty_newsgroups_class_balanced_confirmation_audit.json"
)
...
def validate_newsgroups_length_window_scope(artifact_path: Path, artifact: dict[str, Any]) -> None:
    if artifact.get("source_artifacts") != [str(TWENTY_NEWSGROUPS_ARTIFACT)]:
        raise ValueError(f"{artifact_path} must analyze the committed Twenty Newsgroups artifact")
    dataset = artifact.get("dataset", {})
    if dataset.get("name") != "Twenty Newsgroups" or dataset.get("record_count") != 1998:
        raise ValueError(f"{artifact_path} must use the committed Twenty Newsgroups mini corpus")
    if artifact.get("development_seeds") != [311, 313, 317]:
        raise ValueError(f"{artifact_path} must preserve the development seeds")
    if artifact.get("confirmation_seeds") != [331, 337, 347, 349, 353]:
        raise ValueError(f"{artifact_path} must preserve disjoint confirmation seeds")
    scope = artifact.get("claim_scope", {})
    for key in (
        "real_dataset",
        "metadata_stripped",
        "scan_window_sampled_before_length_selection",
        "post_hoc_development_selection",
        "confirmation_seeds_disjoint",
    ):
        if scope.get(key) is not True:
            raise ValueError(f"{artifact_path} must set {key}")
    for key in (
        "synthetic_domain",
        "heldout_used_for_selection",
        "validation_used_for_selection",
        "teacher_used_for_selection",
        "oracle_train_labels_used_for_selection",
        "paper_ready_claim",
    ):
        if scope.get(key) is not False:
            raise ValueError(f"{artifact_path} must keep {key} false")
    if artifact.get("development_density_win_count") != 0:
        raise ValueError(f"{artifact_path} must preserve the no-development-density-win result")
    if artifact.get("confirmation_same_condition_density_win_count") != 1:
        raise ValueError(f"{artifact_path} must preserve the mixed confirmation result")
    dev_80 = artifact["phases"]["development"]["budgets"]["80"]
    if dev_80.get("best_density_condition") != "length_window_shortest_2x":
        raise ValueError(f"{artifact_path} must preserve the 80-label development best row")
    if dev_80["condition_results"]["length_window_shortest_2x"]["break_even_vs_random"]["candidate_density_wins"]:
        raise ValueError(f"{artifact_path} must preserve the 80-label development density failure")
    confirm_80 = artifact["phases"]["confirmation"]["budgets"]["80"]
    if confirm_80.get("density_win_conditions") != []:
        raise ValueError(f"{artifact_path} must preserve the no 80-label confirmation density win")


def load_newsgroups_length_window_artifact(repo_root: Path) -> dict[str, Any]:
    artifact = load_json(repo_root / TWENTY_NEWSGROUPS_LENGTH_WINDOW_CONFIRMATION_AUDIT_ARTIFACT)
    validate_newsgroups_length_window_scope(
        TWENTY_NEWSGROUPS_LENGTH_WINDOW_CONFIRMATION_AUDIT_ARTIFACT,
        artifact,
    )
    return artifact


def validate_newsgroups_frontier_robustness_scope(artifact_path: Path, artifact: dict[str, Any]) -> None:
    if artifact.get("source_artifacts") != [
        str(TWENTY_NEWSGROUPS_ARTIFACT),
        str(TWENTY_NEWSGROUPS_BUDGETED_ACQUISITION_AUDIT_ARTIFACT),
    ]:
        raise ValueError(f"{artifact_path} must audit the committed Newsgroups frontier artifacts")
    dataset = artifact.get("dataset", {})
    if dataset.get("name") != "Twenty Newsgroups" or dataset.get("record_count") != 1998:
        raise ValueError(f"{artifact_path} must use the committed Twenty Newsgroups mini corpus")
    scope = artifact.get("claim_scope", {})
    for key in ("real_dataset", "metadata_stripped", "paired_seed_audit", "exact_seed_bootstrap", "post_hoc_diagnostic"):
        if scope.get(key) is not True:
            raise ValueError(f"{artifact_path} must set {key}")
    for key in ("synthetic_domain", "introduces_new_policy", "heldout_used_for_selection", "paper_ready_claim"):
        if scope.get(key) is not False:
            raise ValueError(f"{artifact_path} must keep {key} false")
    summary = artifact.get("summary", {})
    if summary.get("comparisons") != 6:
        raise ValueError(f"{artifact_path} must preserve the six paired frontier comparisons")
    if summary.get("mean_density_wins") != 3 or summary.get("robust_density_wins") != 0:
        raise ValueError(f"{artifact_path} must preserve the fragile mean-win result")
    if summary.get("robust_density_losses") != 3:
        raise ValueError(f"{artifact_path} must preserve the robust density-loss count")
    by_name = {row["comparison"]: row for row in artifact.get("comparisons", [])}
    if by_name["class_balanced_80_vs_random"].get("paired_win_count") != 2:
        raise ValueError(f"{artifact_path} must preserve the class-balanced paired win count")
    if by_name["budgeted_margin_2x_160_vs_class_balanced"].get("robust_density_win") is not False:
        raise ValueError(f"{artifact_path} must not promote the budgeted mean win as robust")
    if by_name["prototype_160_vs_random"].get("robust_density_loss") is not True:
        raise ValueError(f"{artifact_path} must preserve the robust prototype density loss")


def load_newsgroups_frontier_robustness_artifact(repo_root: Path) -> dict[str, Any]:
    artifact = load_json(repo_root / TWENTY_NEWSGROUPS_FRONTIER_ROBUSTNESS_AUDIT_ARTIFACT)
    validate_newsgroups_frontier_robustness_scope(
        TWENTY_NEWSGROUPS_FRONTIER_ROBUSTNESS_AUDIT_ARTIFACT,
        artifact,
    )
    return artifact


def validate_newsgroups_frontier_fresh_seed_scope(artifact_path: Path, artifact: dict[str, Any]) -> None:
    dataset = artifact.get("dataset", {})
    if dataset.get("name") != "Twenty Newsgroups" or dataset.get("record_count") != 1998:
        raise ValueError(f"{artifact_path} must use the committed Twenty Newsgroups mini corpus")
    if artifact.get("fresh_seeds") != [439, 443, 449, 457, 461]:
        raise ValueError(f"{artifact_path} must preserve the preregistered fresh seed list")
    scope = artifact.get("claim_scope", {})
    for key in (
        "real_dataset",
        "metadata_stripped",
        "fresh_seed_replication",
        "preregistered_seed_list",
        "paired_seed_audit",
        "exact_seed_bootstrap",
    ):
        if scope.get(key) is not True:
            raise ValueError(f"{artifact_path} must set {key}")
    for key in ("synthetic_domain", "heldout_used_for_selection", "validation_used_for_selection", "paper_ready_claim"):
        if scope.get(key) is not False:
            raise ValueError(f"{artifact_path} must keep {key} false")
    summary = artifact.get("summary", {})
    if summary.get("comparisons") != 6 or summary.get("fresh_seed_count") != 5:
        raise ValueError(f"{artifact_path} must preserve the six five-seed frontier comparisons")
    if summary.get("mean_density_wins") != 1 or summary.get("robust_density_wins") != 0:
        raise ValueError(f"{artifact_path} must preserve the fresh-seed positive-frontier result")
    if summary.get("robust_density_losses") != 2 or summary.get("fragile_mean_density_wins") != 1:
        raise ValueError(f"{artifact_path} must preserve the fresh-seed loss and fragile-win counts")
    by_name = {row["comparison"]: row for row in artifact.get("comparisons", [])}
    class_80 = by_name["class_balanced_80_vs_random"]
    if class_80.get("paired_win_count") != 4 or class_80.get("robust_density_win") is not False:
        raise ValueError(f"{artifact_path} must preserve the fragile class-balanced mean win")
    if class_80.get("sign_test_one_sided_win_p") != 0.1875:
        raise ValueError(f"{artifact_path} must preserve the exact sign-test p-value")
    if by_name["budgeted_margin_2x_160_vs_class_balanced"].get("mean_density_win") is not False:
        raise ValueError(f"{artifact_path} must preserve the budgeted fresh-seed density failure")
    if by_name["prototype_160_vs_random"].get("robust_density_loss") is not True:
        raise ValueError(f"{artifact_path} must preserve the fresh-seed prototype density loss")


def load_newsgroups_frontier_fresh_seed_artifact(repo_root: Path) -> dict[str, Any]:
    artifact = load_json(repo_root / TWENTY_NEWSGROUPS_FRONTIER_FRESH_SEED_AUDIT_ARTIFACT)
    validate_newsgroups_frontier_fresh_seed_scope(
        TWENTY_NEWSGROUPS_FRONTIER_FRESH_SEED_AUDIT_ARTIFACT,
        artifact,
    )
    return artifact


def validate_newsgroups_class_balanced_confirmation_scope(artifact_path: Path, artifact: dict[str, Any]) -> None:
    dataset = artifact.get("dataset", {})
    if dataset.get("name") != "Twenty Newsgroups" or dataset.get("record_count") != 1998:
        raise ValueError(f"{artifact_path} must use the committed Twenty Newsgroups mini corpus")
    expected_seeds = [
    ...
    if artifact.get("confirmation_seeds") != expected_seeds:
        raise ValueError(f"{artifact_path} must preserve the 20-seed confirmation block")
    scope = artifact.get("claim_scope", {})
    for key in ("real_dataset", "metadata_stripped", "targeted_confirmation", "predeclared_seed_block"):
        if scope.get(key) is not True:
            raise ValueError(f"{artifact_path} must set {key}")
    for key in ("synthetic_domain", "introduces_new_policy", "heldout_used_for_selection", "validation_used_for_selection", "paper_ready_claim"):
        if scope.get(key) is not False:
            raise ValueError(f"{artifact_path} must keep {key} false")
    if artifact.get("candidate_condition") != "class_balanced_sample":
        raise ValueError(f"{artifact_path} must preserve the class-balanced candidate")
    if artifact.get("reference_condition") != "random_sample" or artifact.get("train_budget") != 80:
        raise ValueError(f"{artifact_path} must preserve the random 80-label reference comparison")
    summary = artifact.get("summary", {})
    if summary.get("seed_count") != 20 or summary.get("paired_win_count") != 10:
        raise ValueError(f"{artifact_path} must preserve the 10/20 paired confirmation result")
    if summary.get("robust_density_win") is not False:
        raise ValueError(f"{artifact_path} must not promote the confirmation row")
    if summary.get("sign_test_one_sided_win_p") != 0.588099:
        raise ValueError(f"{artifact_path} must preserve the exact sign-test p-value")
    if summary.get("bootstrap", {}).get("density_delta_ci95") != [-0.002197, 0.003466]:
        raise ValueError(f"{artifact_path} must preserve the bootstrap interval")


def load_newsgroups_class_balanced_confirmation_artifact(repo_root: Path) -> dict[str, Any]:
    artifact = load_json(repo_root / TWENTY_NEWSGROUPS_CLASS_BALANCED_CONFIRMATION_AUDIT_ARTIFACT)
    validate_newsgroups_class_balanced_confirmation_scope(
        TWENTY_NEWSGROUPS_CLASS_BALANCED_CONFIRMATION_AUDIT_ARTIFACT,
        artifact,
    )
    return artifact
...
def build_macros(repo_root: Path) -> str:
    ...
    load_newsgroups_length_window_artifact(repo_root)
    load_newsgroups_frontier_robustness_artifact(repo_root)
    load_newsgroups_frontier_fresh_seed_artifact(repo_root)
    load_newsgroups_class_balanced_confirmation_artifact(repo_root)
    ...
    lines = [
        ...
        rf"\newcommand{{\LsdPaperArtifactCount}}{{{len(BUDGET_ARTIFACTS) + len(support) + len(real_text_artifacts) + 12}}}",
...
def build_twenty_newsgroups_length_window_table(repo_root: Path) -> str:
    artifact = load_newsgroups_length_window_artifact(repo_root)
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Twenty Newsgroups sampled length-window stress audit. Development rows select the best length-window policy by density on seeds 311, 313, and 317; confirmation rows evaluate that same development-selected policy on disjoint seeds 331, 337, 347, 349, and 353. The 80-label near-frontier does not beat same-phase random after confirmation.}",
        r"\label{tab:twenty-newsgroups-length-window-audit}",
        r"\scriptsize",
        r"\setlength{\tabcolsep}{3pt}",
        r"\begin{tabular}{@{}rlrrrrr@{}}",
        r"\toprule",
        r"Budget & Dev-selected policy & Dev LSD & Dev random & Conf. LSD & Conf. random & Conf. win \\",
        r"\midrule",
    ]
    for budget in artifact["train_budgets"]:
        dev_budget = artifact["phases"]["development"]["budgets"][str(budget)]
        confirm_budget = artifact["phases"]["confirmation"]["budgets"][str(budget)]
        condition = dev_budget["best_density_condition"]
        dev_row = dev_budget["condition_results"][condition]
        confirm_row = confirm_budget["condition_results"][condition]
        label = condition.replace("length_window_", "").replace("_", " ")
        lines.append(
            " & ".join(
                [
                    str(budget),
                    latex_escape(label),
                    fmt_float(dev_row["signed_learning_signal_density_per_1m_event_compute_mean"], digits=6),
                    fmt_float(dev_budget["random_reference"]["signed_learning_signal_density_per_1m_event_compute_mean"], digits=6),
                    fmt_float(confirm_row["signed_learning_signal_density_per_1m_event_compute_mean"], digits=6),
                    fmt_float(
                        confirm_budget["random_reference"]["signed_learning_signal_density_per_1m_event_compute_mean"],
                        digits=6,
                    ),
                    latex_escape(str(confirm_row["break_even_vs_random"]["candidate_density_wins"])),
                ]
            )
            + r" \\"
        )
...
def build_twenty_newsgroups_frontier_robustness_table(repo_root: Path) -> str:
    artifact = load_newsgroups_frontier_robustness_artifact(repo_root)
    labels = {
        "class_balanced_80_vs_random": "Class-balanced 80 vs random",
        "prototype_40_vs_random": "Prototype 40 vs random",
        "prototype_160_vs_random": "Prototype 160 vs random",
        "budgeted_margin_1x_160_vs_class_balanced": "Budgeted margin 1x vs class",
        "budgeted_margin_2x_160_vs_class_balanced": "Budgeted margin 2x vs class",
        "budgeted_margin_2x_160_vs_random": "Budgeted margin 2x vs random",
    }
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Twenty Newsgroups paired-seed frontier robustness audit. Mean density wins are checked with exact seed-level bootstrap intervals over the paired seeds. No current positive frontier clears the conservative robust-win rule; prototype retrieval losses against random are robust under this audit.}",
        r"\label{tab:twenty-newsgroups-frontier-robustness}",
        r"\scriptsize",
        r"\setlength{\tabcolsep}{3pt}",
        r"\begin{tabular}{@{}lrrrrrl@{}}",
        r"\toprule",
        r"Comparison & Budget & Mean ratio & Paired wins & CI low & CI high & Reading \\",
        r"\midrule",
    ]
    for row in artifact["comparisons"]:
        ci_low, ci_high = row["bootstrap"]["density_delta_ci95"]
        ...
        lines.append(
            " & ".join(
                [
                    latex_escape(labels[row["comparison"]]),
                    str(row["budget"]),
                    fmt_float(row["mean_density_ratio"], digits=3),
                    f"{row['paired_win_count']}/{row['seed_count']}",
                    fmt_float(ci_low, digits=6),
                    fmt_float(ci_high, digits=6),
                    latex_escape(reading),
                ]
            )
            + r" \\"
...
def build_twenty_newsgroups_frontier_fresh_seed_table(repo_root: Path) -> str:
    artifact = load_newsgroups_frontier_fresh_seed_artifact(repo_root)
    labels = {
        "class_balanced_80_vs_random": "Class-balanced 80 vs random",
        "prototype_40_vs_random": "Prototype 40 vs random",
        "prototype_160_vs_random": "Prototype 160 vs random",
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Twenty Newsgroups fresh-seed frontier audit on untouched seeds 439, 443, 449, 457, and 461. No positive frontier clears the conservative robust-win rule; the base 80-document class-balanced row remains a fragile mean win, while prototype-160 and budgeted margin-2x losses against random remain robust under charged compute.}",
        r"\label{tab:twenty-newsgroups-frontier-fresh-seed}",
        r"\scriptsize",
        r"\setlength{\tabcolsep}{3pt}",
        r"\begin{tabular}{@{}lrrrrrl@{}}",
        r"\toprule",
        r"Comparison & Mean ratio & Paired wins & CI low & CI high & Sign $p$ & Reading \\",
        r"\midrule",
    ]
    for row in artifact["comparisons"]:
        ci_low, ci_high = row["bootstrap"]["density_delta_ci95"]
        if row["robust_density_win"]:
            reading = "robust win"
            sign_p = row["sign_test_one_sided_win_p"]
        elif row["robust_density_loss"]:
            reading = "robust loss"
            sign_p = row["sign_test_one_sided_loss_p"]
        elif row["mean_density_win"]:
            reading = "fragile mean win"
            sign_p = row["sign_test_one_sided_win_p"]
        else:
...
def build_twenty_newsgroups_class_balanced_confirmation_table(repo_root: Path) -> str:
    artifact = load_newsgroups_class_balanced_confirmation_artifact(repo_root)
    summary = artifact["summary"]
    ci_low, ci_high = summary["bootstrap"]["density_delta_ci95"]
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Twenty Newsgroups targeted 20-seed confirmation for the best remaining positive frontier candidate. The class-balanced 80-document row keeps a small mean density edge over random, but wins only half the paired seeds and fails both the bootstrap-interval and sign-test criteria.}",
        r"\label{tab:twenty-newsgroups-class-balanced-confirmation}",
        r"\scriptsize",
        r"\setlength{\tabcolsep}{4pt}",
        r"\begin{tabular}{@{}lrrrrrr@{}}",
        r"\toprule",
        r"Candidate & Budget & Mean ratio & Paired wins & CI low & CI high & Sign $p$ \\",
        r"\midrule",
        " & ".join(
            [
                "Class-balanced vs random",
                str(artifact["train_budget"]),
                fmt_float(summary["mean_density_ratio"], digits=3),
                f"{summary['paired_win_count']}/{summary['seed_count']}",
                fmt_float(ci_low, digits=6),
                fmt_float(ci_high, digits=6),
                fmt_float(summary["sign_test_one_sided_win_p"], digits=6),
            ]
        )
        + r" \\",
        r"\bottomrule",
...
def render_tables(repo_root: Path) -> str:
    return "\n".join(
        [
            ...
            build_twenty_newsgroups_length_window_table(repo_root),
            build_real_text_break_even_certificate_table(repo_root),
            build_twenty_newsgroups_frontier_robustness_table(repo_root),
            build_twenty_newsgroups_frontier_fresh_seed_table(repo_root),
            build_twenty_newsgroups_class_balanced_confirmation_table(repo_root),
