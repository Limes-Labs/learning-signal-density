...
def _add_budgeted_acquisition_rows(rows: list[dict], artifact: dict) -> None:
    for budget in artifact["train_budgets"]:
        budget_key = str(budget)
        for condition, stats in artifact["budgets"][budget_key]["condition_results"].items():
            for reference_name, break_even_key in (
                ...
                rows.append(
                    _copy_row(
                        family="twenty_newsgroups_budgeted_active_acquisition",
                        artifact_label="Twenty Newsgroups budgeted acquisition",
                        source_artifact=str(TWENTY_NEWSGROUPS_BUDGETED_ACQUISITION_AUDIT_ARTIFACT),
                        budget=budget_key,
                        reference=reference_name,
                        candidate=condition,
                        row=row,
                        note="budgeted-window active label acquisition against true-label sampling",
                    )
                )
...
def build_real_text_break_even_certificate(repo_root: Path) -> dict:
    ...
    budgeted_acquisition = load_json(repo_root / TWENTY_NEWSGROUPS_BUDGETED_ACQUISITION_AUDIT_ARTIFACT)
    ...
    _add_budgeted_acquisition_rows(rows, budgeted_acquisition)
